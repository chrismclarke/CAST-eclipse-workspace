...
public class SampSeVsSdApplet extends XApplet {
...
*/		
			NormalDistnVariable estErrorDistn = new NormalDistnVariable(getParameter(ERROR_NAME_PARAM));
...
*/	
	...
	private XPanel dataPanel(DataSet data) {
		...
		thePanel.setLayout(new AxisLayout());
			...
			theHorizAxis.setForeground(kPopnColor);
		thePanel.add("Bottom", theHorizAxis);
		
			StackedDotPlotView dataView = new StackedDotPlotView(data, this, theHorizAxis);
			dataView.setActiveNumVariable("y");
			dataView.lockBackground(Color.white);
			dataView.setForeground(kPopnColor);
	...
	private XPanel errorDistnPanel(SummaryDataSet summaryData) {
		...
		thePanel.setLayout(new AxisLayout());
			...
			HorizAxis theHorizAxis = getAxis(summaryData, "estErrorDistn", ERROR_INFO_PARAM);
			theHorizAxis.setForeground(kErrorColor);
		thePanel.add("Bottom", theHorizAxis);
		
			SimpleDistnView errorDistnView = new SimpleDistnView(summaryData, this, theHorizAxis, "estErrorDistn");
			errorDistnView.setDensityScaling(0.9);
			errorDistnView.lockBackground(Color.white);
			errorDistnView.setForeground(kErrorColor);
			errorDistnView.setDensityColor(kNormalErrorColor);
...
//			c.ipadx = 20;
		...
		thePanel.add(seLabel, c);
			...
			c.gridy = 1;
			c.insets = leftInsets;
			XLabel sampleLabel = new XLabel(translate("Sample estimate"), XLabel.LEFT, this);
		...
		thePanel.add(sdEstView, c);
			...
			c.gridx = 2;
			c.gridy = 1;
			StdErrorValueView seEstView = new StdErrorValueView(summaryData, this, "estErrorDistn", true);
